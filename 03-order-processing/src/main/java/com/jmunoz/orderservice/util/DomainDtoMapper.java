package com.jmunoz.orderservice.util;
...
public class DomainDtoMapper {
    ...
    // Este sería el escenario de camino feliz.
    // En caso de error, esto lo vamos a gestionar usando un controller advice.
    ...
    private static List<OrderResponse.Product> toProducts(Order order) {
        ...
        return switch (order.orderItem().product()) {
            ...
            case Bundle bundle -> bundle.items()
                    .stream()
                    .map((Single single) -> toProduct(single, quantity))
                    .toList();
        };
    }
